...
```
Mean steps per day = `r daymean`
Median steps per day = `r daymed`
...
```
Maximum number of steps in an interval is `r maxrow$steps`, which occurs at interval `r intmax`.
...
```
The mean value of steps per day, with NA values imputed is `r daymeannona`,
    the same value as the previously reported mean, `r daymean`.
The median value of steps per day, with NA values imputed is `r daymednona`,
    just a small increase from its previous value, `r daymed`.
